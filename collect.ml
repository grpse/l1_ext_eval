	...
	let rec collectWithTypeVarNumber env e varNumber =
		...
		|	Binop(op, e1, e2) -> 
			(
				...
				|	(Gt,   _, _) -> (Tbool,  c1 @ c2 @ [(t1, Tint);  (t2, Tint)])
				|	(Lt,   _, _) -> (Tbool,  c1 @ c2 @ [(t1, Tint);  (t2, Tint)])
				|	(Ge,   _, _) -> (Tbool,  c1 @ c2 @ [(t1, Tint);  (t2, Tint)])
				|	(Le,   _, _) -> (Tbool,  c1 @ c2 @ [(t1, Tint);  (t2, Tint)])
				|	(Eq,   _, _) -> (Tbool,  c1 @ c2 @ [(t1, Tint);  (t2, Tint)])
				|	(Df,   _, _) -> (Tbool,  c1 @ c2 @ [(t1, Tint);  (t2, Tint)])
